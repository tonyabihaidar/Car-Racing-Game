...
const AESSingleBlockDecryption: React.FC = () => {
  ...
  const [ciphertext, setCiphertext] = useState("3925841d02dc09fbdc118597196a0b32");
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.text("AES Single-Block Decryption - Round Keys & States", pageWidth / 2, yPos, {
        align: "center",
      });
      yPos += 15;
      ...
      yPos += 6;
      doc.text(`Plaintext: ${formatOutput(result.ciphertext)}`, 20, yPos);
      yPos += 12;
      ...
      doc.save(`AES-${keySize}_single-block-decryption_rounds.pdf`);
  ...
  const handleDecrypt = () => {
    try {
      ...
      if (!hexOnlyRegex.test(ciphertext.replace(/\s+/g, "")) || ciphertext.replace(/\s+/g, "").length !== 32) {
        throw new Error("Ciphertext must be exactly 32 hex characters.");
      }
      ...
      const ctBytes = hexToBytes(ciphertext.replace(/\s+/g, ""));
      
      const decryptResult = aesDecrypt(ctBytes, keyBytes);

      setResult(decryptResult);
      setCurrentStepIndex(0);
      showToast("Block decrypted.");
    } catch (err: any) {
    ...
    <div className={`aes-root ${theme === "dark" ? "aes-root--dark" : "aes-root--light"}`}>
      ...
      <header className="aes-header" style={{ flexDirection: "column", alignItems: "flex-start", gap: "8px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px", width: "100%" }}>
          ...
          <div>
            <h1 className="aes-title">Single-Block AES Decryption</h1>
            <p className="aes-subtitle">
              Decrypt a single 16-byte AES block with step-by-step visualization.
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="aes-layout">
        ...
        <section className="aes-panel aes-panel--left">
          ...
          <div className="aes-form-group">
            <label className="aes-label">
              Ciphertext (hex, 32 characters)
              <textarea
                className="aes-textarea"
                rows={3}
                value={ciphertext}
                onChange={(e) => setCiphertext(e.target.value)}
                placeholder="Enter 32 hex characters to decrypt..."
              />
            </label>
            <p className="aes-help">
              The encrypted data you want to decrypt back to plaintext.
            </p>
          </div>
        ...
        <section className="aes-panel aes-panel--right">
          <div className="aes-steps-header">
            <div>
              ...
              {hasVisualSteps && currentStep ? (
                ...
                <p className="aes-steps-subtitle">
                  Decrypt a block to visualize the process.
                </p>
              )}
            </div>
          ...
          <div className="aes-cipher-section">
            <div className="aes-cipher-header">
              <span className="aes-result-label">Plaintext ({outputFormat})</span>
                ...
                <div className="aes-legend-item aes-legend-item--state">
                  ...
                  <div>
                    ...
                    <div className="aes-legend-text">
                      4×4 bytes representing the internal AES state during decryption.
                    </div>
                  </div>
                </div>
...
export default AESSingleBlockDecryption;